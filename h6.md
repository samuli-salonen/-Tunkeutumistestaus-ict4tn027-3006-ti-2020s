...
Tässä tehtävässä piti murtaa 5 hash:ia hashcat:llä. Murrot tein kali linux:lla. Salasanatiivisteet tein seuraavasti: MD5 Apche2 palvelimella luomalla .htpasswd tiedoston, muut tein ```mkpasswd```:llä. En saanut asennettua ROCm (AMD gpu), enkä OpenCL Runtime for Intel Core:a, sillä kali valitti rikkinäisistä paketeista. (Kyllä olin päivittänyt repot ja paketit ennen asennusyrityksiä). Tässä tehtävässä mennään siis ei optimoidulla kernelillä ja prossun tehoilla crack:ta salasanoja. Tämä vie aikaa kauemmin kuin GPU:lla, sillä nykyiset gpu:t ovat tehokkaita ja ominaisuuksiltaan parempia suorittamaan esimerkiksi salasanamurtoja. 
...
Komento on sama kuin sha256-murrossa. Kokeilin lippua: ```-T 3```, eli yritin nostaa prossun threadien määrää ajaakseni murron nopeammin.
...
Ja salasana löytyi taas, se on tässä tapauksessa: ```abc```!
