...
Tässä tehtävässä piti murtaa esimerkiksi zipattu ja salasanasuojattu tiedosto. Päätin luoda salasanasuojatun ```.zip``` tiedoston zip:llä käyttäen komentoa:```zip -P SALASANA salaisiatietoja.zip salaisiatietoja.txt```. Minulla oli etukäteen tekstitiedosto, minkä zippasin ja salasin. Komennossa ```-P``` tarkoittaa salasana suojausta, jonka vaihdamme SALASANA tilalle. Komennossa ```salaisiatietoja.zip salaisiatietoja.txt``` ensimmäiseksi määritellään luotava tiedosto ja lopussa on salattava tiedosto, minkä haluamme pakata ja salata.
...
Ja saimme tiivisteen, nyt voimme alkaa murtaa sitä vertaamalla salasanalistaan.

Ajamme hash:n vertailu tekemääni sanalistaa vasten. Komento jolla vertaamme on seuraava:```john -wordlist=password.lst asdf01.txt```
