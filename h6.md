...
Syötämme ```username:``` kohtaan "kayttaja" ja ```Password:``` kohtaa "salasana", sitten painamme burpsuitesta intercept:n päälle ja sivulta painamme "go". Saimme HTTP historiaan burpsuitessa seuraavanlaisen POST-request:n:
...
Sitten voimme alkaa rakentaa hydra komentoa, jolla tunkeudumme järjestelmään (web-lomakkeeseen). Päätin rakentaa komennoksi seuraavanlaisen komennon:```hydra```
