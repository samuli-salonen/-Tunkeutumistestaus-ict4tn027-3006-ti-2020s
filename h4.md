...
Tässä tehtävässä päätin ottaa yhden walktrough:n ajan säästämiseksi. Päätin kokeilla yhtä työkalua.
...
Kuvitellaan, että ollaan murtauduttu ei root käyttäjälle pentest-servulle ja käyttöoikeudet ovat rajalliset. Olemme käyttäjällä foo ja haluamme löytää koneelta rootiksi pääsemiseen mahdollistavia tiedostoja ja prosesseja. Lataamme siis LinPeas.sh komennolla:```wget https://raw.githubusercontent.com/carlospolop/privilege-escalation-awesome-scripts-suite/master/linPEAS/linpeas.sh```.
