...
Tässä tehtävässä lähdin avaamalla yhteyden HTB:n verkkoon ja varmistamalla taas, että ollaan oikeassa verkossa samalla tavalla kuin aiempien viikkotehtävienkin kanssa. Tätä VPN tunneli kohtaa en raportoi, koska se on jo käsitelty aiemmissa raporteissa. Raportoin vain HackTheBox:n verkon skannaustulokset.

Kun sain yhteyden HTB:n verkkoon aloitin tekemällä komennon ```sudo msfconsole```, tällä pääsimme metasploitin consoliin, jossa teemme komennot, että saamme luotua hakkerointi tarpeisiimme tietokantoja. (Tällä pyritään siis organisoimaan penaustestaukset).
